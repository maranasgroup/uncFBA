...
*       Author: Hoang Dinh
* Searching for non-zero RHS terms that can lead to improved growth yield 
*************************************************************************************
...
v.up('R__BIOMASS_Ec_iML1515_core_75p37M') = 1000;
...
Model maxBiomfromRHS
/Obj, Stoic, Elem/;
maxBiomfromRHS.optfile = 1;

******************* SOLVE *******************
Solve maxBiomfromRHS using lp maximizing z;

******************* WRITE TO TXT FILE *******************
file ff /pulp_maxBiom.sl_vals.txt/;
put ff;
loop(i$metint(i),
	if ( (slL.l(i) gt 1E-9),
...
file ff2 /pulp_maxBiom.modelStat.txt/;
put ff2;
put maxBiomfromRHS.modelStat/;
putclose ff2;

file ff3 /pulp_maxBiom.maxGR.txt/;
put ff3;
put v.l('R__BIOMASS_Ec_iML1515_core_75p37M'):0:8/;
putclose ff3;
